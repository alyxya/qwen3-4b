...
class Qwen3Model(nn.Module):
    ...
    def generate(
        ...
        Args:
            ...
            top_k: Top-k sampling (if None, not used)
            eos_token_id: Token ID that signals end of generation

        Returns:
            Generated token IDs, shape (batch_size, seq_len + num_generated)
        """
        batch_size, seq_len = input_ids.shape
        device = input_ids.device

        # Initialize caches as None (will be populated during first forward pass)
        cache_k = None
        cache_v = None
        ...
        for _ in range(max_new_tokens):
            # For first iteration, pass all tokens. For subsequent iterations, only pass the last token.
            if cache_k is None:
                # First forward pass: process entire prompt
                forward_input = generated
            else:
                # Subsequent passes: only process the last generated token
                forward_input = generated[:, -1:]

            # Forward pass
            logits, cache_k, cache_v = self.forward(
                forward_input,
                cache_k=cache_k,
                cache_v=cache_v,
                return_logits=True,
            ...
            # logits: (batch_size, 1, vocab_size) or (batch_size, seq_len, vocab_size)
